...
> **阅读优先级：**
> 1. **术语对齐**：必须首先阅读 `docs/terminology.md` 以确保使用正确的命名空间（Metric, Signal, Integration Data）。**禁止**在非 API 限制场景下使用 "Quota" 一词。
...
Glancier (原 Quota Board) 是一个**配置驱动**的**个人数据聚合器与中心 (Personal Data Aggregator & Hub)**。核心理念是：**用户只需配置 Flow (Integration) + View Templates，即可接入并聚合任意第三方 API 或网页数据**，并在基于 **Bento Grid** 哲学的看板中实时展示各项指标 (Metrics) 与信号 (Signals)。
...
2. **术语一致性**：所有新功能开发必须遵循 `docs/terminology.md`。
    ...
    - **Progress Bar**：替代原 Quota Bar。
    - **Bento Card**：替代原 Quota Card。
...
**强制规则**：
- **环境变量**：使用 `GLANCIER_DATA_DIR` 替代原 `QUOTA_BOARD_ROOT`。
- **后端**：
    - 新增鉴权 → `core/auth/`
    - 新增解析 → `core/parser.py`
    - 新增 API → `core/api.py`
